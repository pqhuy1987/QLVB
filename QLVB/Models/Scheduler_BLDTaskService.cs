...
{
    public class Scheduler_BLDTaskService : ISchedulerEventService<TaskViewModel>
    {
        private static bool UpdateDatabase = true;
        private QuanLyVanBanEntities db;

        public Scheduler_BLDTaskService(QuanLyVanBanEntities context)
        {
            db = context;
        }

        public Scheduler_BLDTaskService()
        ...
        public virtual IList<TaskViewModel> GetAll()
        {
            ...
            {
                result = db.LichHop_BLD.ToList().Select(task => new TaskViewModel
                {
                    TaskID = task.ID,
                    Title = task.Subject,
        ...
        public virtual void Insert(TaskViewModel task, ModelStateDictionary modelState)
        {
            if (ValidateModel(task, modelState))
            {
                ...
                else
                {
                    ...
                    var entity = task.ToEntityBLD();

                    db.LichHop_BLD.Add(entity);
                    db.SaveChanges();
        ...
        public virtual void Delete(TaskViewModel task, ModelStateDictionary modelState)
        {
            ...
            else
            {
                ...
                db.LichHop_BLD.Attach(entity);
                 string itaskID = task.TaskID.ToString();
                 var recurrenceExceptions = db.LichHop_BLD.Where(t => t.RecurrenceParentId == itaskID);

                foreach (var recurrenceException in recurrenceExceptions)
                {
                    db.LichHop_BLD.Remove(recurrenceException);
                }

                db.LichHop_BLD.Remove(entity);
                db.SaveChanges();
            }
        }
